...
function isCompleteMint(mintId: string): boolean {
  ...
  // sufficient checks
  if (mintEvent != null) return mintEvent.sender !== null;
  else return false;
}
...
export function handleSync(event: Sync): void {
  ...
  if (pair != null) {
    ...
    if (uniswap != null && token0 != null && token1 != null) {
      ...
      if (bundle != null) {
        ...
        token0.derivedETH = findEthPerToken(token0 as Token) as BigDecimal;
        token1.derivedETH = findEthPerToken(token1 as Token) as BigDecimal;
        token0.save();
        token1.save();
...
export function handleBurn(event: Burn): void {
  ...
  let pair = Pair.load(event.address.toHex());
  let uniswap = UniswapFactory.load(FACTORY_ADDRESS);
  if (pair != null && burn != null && uniswap != null) {
    //update token info
    let token0 = Token.load(pair.token0);
    let token1 = Token.load(pair.token1);
    if (token0 != null && token1 != null) {
  ...
  if (pair != null) {
    ...
    if (token0 != null && token1 != null) {
      ...
      if (bundle != null) {
        ...
        token0.tradeVolume = token0.tradeVolume.plus(
          amount0In.plus(amount0Out)
        );
        ...
        token1.tradeVolume = token1.tradeVolume.plus(
          amount1In.plus(amount1Out)
        );
        ...
        pair.untrackedVolumeUSD = pair.untrackedVolumeUSD.plus(
          derivedAmountUSD
        );
        pair.txCount = pair.txCount.plus(ONE_BI);
        pair.save();
        ...
        let uniswap = UniswapFactory.load(FACTORY_ADDRESS);
        if (uniswap != null) {
          uniswap.totalVolumeUSD = uniswap.totalVolumeUSD.plus(
            trackedAmountUSD
          );
          uniswap.totalVolumeETH = uniswap.totalVolumeETH.plus(
            trackedAmountETH
          );
        ...
        if (token0DayData != null && token1DayData != null) {
